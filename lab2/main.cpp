...
void drawCircle(float radius, float centerX, float centerY) {
    int numSegments = 100;
    GLfloat circleVertices[2 * (numSegments + 1)];
    ...
    for (int i = 0; i <= numSegments; i++) {
        ...
        circleVertices[2 * i] = x + centerX;
        circleVertices[2 * i + 1] = y + centerY;
    }

    glEnableClientState(GL_VERTEX_ARRAY);//включение исп массива вершин
    glVertexPointer(2, GL_FLOAT, 0, circleVertices);//опред массив вершин

    glDrawArrays(GL_POLYGON, 0, numSegments + 1);

    glDisableClientState(GL_VERTEX_ARRAY);
}

void drawSquare(float x1, float x2, float y1, float y2) {
    GLfloat squareVertices[] = {x1, y2, x2, y2, x2, y1, x1, y1};
